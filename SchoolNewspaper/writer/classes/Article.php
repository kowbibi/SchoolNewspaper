...
class Article extends Database {
    ...
    public function getArticles($id = null) {
        ...
        $sql = "SELECT articles.*, school_publication_users.username, COALESCE(categories.name, '') AS category_name
                FROM articles 
    ...
    public function getActiveArticles($id = null) {
        ...
        $sql = "SELECT articles.*, school_publication_users.username, school_publication_users.is_admin AS is_admin, COALESCE(categories.name, '') AS category_name FROM articles 
    ...
    public function getArticlesByUserID($user_id) {
        $sql = "SELECT articles.*, school_publication_users.username, COALESCE(categories.name, '') AS category_name FROM articles 
                JOIN school_publication_users ON 
                articles.author_id = school_publication_users.user_id
                LEFT JOIN categories ON articles.category_id = categories.category_id
    ...
    public function getSharedWithUser($user_id) {
        $sql = "SELECT a.*, u.username, COALESCE(c.name, '') AS category_name 
                ...
                WHERE s.shared_with_user_id = ? ORDER BY a.created_at DESC";
        return $this->executeQuery($sql, [$user_id]);
    }
    ...
    public function hasPendingEditRequest($article_id, $requester_id) {
        $row = $this->executeQuerySingle("SELECT request_id FROM edit_requests WHERE article_id = ? AND requester_id = ? AND status = 'pending'", [$article_id, $requester_id]);
        return !empty($row);
    }
}
?>